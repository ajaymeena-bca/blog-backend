...
import { NewBlogRquestBody, UpdateBlogRequestBody } from "../types/types.js";
import ErrorHandler from "../utils/utility-class.js";
import { Blog } from "../models/blog.js";


export const newBlog = TryCatch(async(
    req:Request<{},{},NewBlogRquestBody>,
    res,next
)=>{
     ...
     if(!context) return next(new ErrorHandler("context is required", 400));
...
export const getAllBlog = TryCatch(async(req,res,next)=>{
      ...
      if(blogs.length==0) return next(new ErrorHandler("Blogs are not exist", 400));
...
export const getCategories = TryCatch(async(req,res,next)=>{
      ...
      if(categories.length==0) return next(new ErrorHandler("category are not exist", 400));
