...
export const newComment = TryCatch(async (req, res, next) => {
    ...
    if (!context)
        return next(new ErrorHandler("context is not provided", 400));
    if (!blog)
        return next(new ErrorHandler("blog refference is not given", 400));
    console.log(context);
...
export const allComment = TryCatch(async (req, res, next) => {
...
export const singleBlogComment = TryCatch(async (req, res, next) => {
